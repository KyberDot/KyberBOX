...
const db = require('../db');
const { runRestartCommand } = require('../utils/ssh');
const { sendMail } = require('../utils/mailer');
const { getAllSettings, getSiteBaseUrl } = require('../utils/settings');

const router = express.Router();
const RESTART_COOLDOWN_MS = 6 * 60 * 60 * 1000; // once every 6 hours
...
router.get('/dashboard', (req, res) => {
  ...
  const hasPlex = subscriptions.some((s) => ['plex', 'multiple'].includes(s.service) && s.status === 'active');
  const sshTarget = db.prepare('SELECT * FROM ssh_targets WHERE user_id = ?').get(req.user.id);

  const lastRestart = db
    .prepare('SELECT * FROM restart_log WHERE user_id = ? ORDER BY requested_at DESC LIMIT 1')
    .get(req.user.id);

  let nextAllowedAt = null;
  if (lastRestart) {
    const last = new Date(lastRestart.requested_at + 'Z').getTime();
    const next = last + RESTART_COOLDOWN_MS;
...
router.post('/dashboard/plex-restart', async (req, res) => {
  const subscriptions = db.prepare('SELECT * FROM subscriptions WHERE user_id = ?').all(req.user.id);
  const hasPlex = subscriptions.some((s) => ['plex', 'multiple'].includes(s.service) && s.status === 'active');

  if (!hasPlex) {
    return res.status(403).json({ ok: false, message: 'No active Plex subscription on this account.' });
  }
  ...
  if (lastRestart) {
    ...
    if (Date.now() - last < RESTART_COOLDOWN_MS) {
      const waitMs = RESTART_COOLDOWN_MS - (Date.now() - last);
      const waitMins = Math.ceil(waitMs / 60000);
      return res.status(429).json({
        ok: false,
        message: `You can only restart Plex once every 6 hours. Please wait about ${waitMins} more minute(s).`,
      });
    }
  }

  const target = db.prepare('SELECT * FROM ssh_targets WHERE user_id = ?').get(req.user.id);
  if (!target) {
    db.prepare('INSERT INTO restart_log (user_id, success, output) VALUES (?, 0, ?)').run(
      req.user.id,
      'No server access configured yet - contact support.'
    );
    return res.status(400).json({
      ok: false,
      message: 'Your server access has not been configured yet. Please raise a ticket so we can set that up.',
    });
  }

  const result = await runRestartCommand(target);

  db.prepare('INSERT INTO restart_log (user_id, success, output) VALUES (?, ?, ?)').run(
    req.user.id,
    result.success ? 1 : 0,
    result.output
  );
  ...
  res.json({
    ...
    message: result.success
      ? 'Restart command sent successfully. Plex should be back shortly.'
      : `Restart failed: ${result.output}`,
  });
});
